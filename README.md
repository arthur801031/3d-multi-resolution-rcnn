...
Before mmdetection can work, please replace your package's `cocoeval.py` with `backups/cocoeval.py` and `coco.py` with `backups/coco.py`.

## Training and Testing Commands
```bash
################## train ##################  
python setup.py install && CUDA_VISIBLE_DEVICES=0 ./tools/dist_train.sh work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny-2/bbox-segm-rpnthresh-2maskheads/config.py 1 --validate --resume_from work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny-2/bbox-segm-rpnthresh-2maskheads/epoch_1353.pth
# rebuild && CUDA_DEVICE(s) train_script config_path number_of_gpu whether_or_not_to_perform_validation resume_from_checkpoint_path

################## test ##################
python setup.py install && CUDA_VISIBLE_DEVICES=1 python tools/test.py work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny-2/bbox0.69-segm-rpnthresh-2maskheads/config.py work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny-2/bbox0.69-segm-rpnthresh-2maskheads/epoch_1560.pth --gpus 1 --out results.pkl --eval bbox segm
# rebuild && CUDA_DEVICE(s) python test_code config_path checkpoint_path --gpus number_of_gpu --out output_filename --eval bounding box and/or segmentation
```

## Image Preprocessing
For 3D Mask R-CNN and proposed model, use `Preprocessing-COCODataset-v3-New-Label.ipynb`.

For 2D Mask R-CNN, use `Preprocessing-COCODataset-v1-New-Label.ipynb`.

For 2D Mask R-CNN RGB, use `Preprocessing-COCODataset-v2-New-Label.ipynb`.
...
Proposed model: 
```
work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny-2/bbox0.69-segm-rpnthresh-2maskheads
```

3D Mask R-CNN: 
```
work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny-2/bbox0.55-segm-baseline-roifixed-best566epoch
```

3D Mask R-CNN with Multi-Resolution Approach and Architectural Augmentation (1x and 1.5x):
```
work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny-2/bbox0.64-segm-2maskheads
```

3D Faster R-CNN:
```
work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny/bboxonly52.6-roifixed-best706epoch
```

3D Faster R-CNN with Multi-Resolution Approach and Architectural Augmentation (1x and 1.5x):
```
work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny/bbox0.64-bboxonly-2scalesheads-best975
```

3D Faster R-CNN with Multi-Resolution Approach and Architectural Augmentation and RPN Soft-Cutoff Threshold (1x and 1.5x): 
```
work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny/bbox0.66-bboxonly-rpnthresh-2scalesheads
```

3D Faster R-CNN with Multi-Resolution Approach and Architectural Augmentation and RPN Soft-Cutoff Threshold (1x and 1.25x): 
```
work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny/bbox0.64-bboxonly-1dot25x-2scalesheads-best2230
```

3d Faster R-CNN Two Scales with One Pathway: 
```
work_dirs/checkpoints/ym_mask_rcnn_r50_fpn_1x_3d_tiny/bbox0.58-bboxly-2scalesmodel-bestepoch328/config.py
```


## Generate Visualization Results
3D: `Evaluate-Validation-Images-3D.ipynb`

2D: `Evaluate-Validation-Images.ipynb`




# Orginal mmdetection instructions:
