...
class Coco3D3ScalesDataset(CustomDataset):

    CLASSES = ('microbleed')

    def load_annotations(self, ann_file, ann_file_2=None, ann_file_3=None):
        if ann_file_2 is None:
            ann_file_2 = ann_file
        if ann_file_3 is None:
            ann_file_3 = ann_file

        self.coco = COCO(ann_file)
        self.coco_2 = COCO(ann_file_2)
        self.coco_3 = COCO(ann_file_3)

        self.cat_ids = self.coco.getCatIds()
        self.cat_ids_2 = self.coco_2.getCatIds()
        self.cat_ids_3 = self.coco_3.getCatIds()
        ...
        self.cat2label_3 = {
            cat_id: i + 1
            for i, cat_id in enumerate(self.cat_ids_3)
        }

        self.img_ids = self.coco.getImgIds()
        self.img_ids_2 = self.coco_2.getImgIds()
        self.img_ids_3 = self.coco_3.getImgIds()

        img_infos, img_infos_2, img_infos_3 = [], [], []
        ...
        for i in self.img_ids_3:
            info = self.coco_3.loadImgs([i])[0]
            info['filename'] = info['file_name']
            img_infos_3.append(info)
        return img_infos, img_infos_2, img_infos_3
    ...
    def get_ann_infos(self, idx):
        ...
        img_id_3 = self.img_infos_3[idx]['id']
        ann_ids_3 = self.coco_3.getAnnIds(imgIds=[img_id_3])
        ann_info_3 = self.coco_3.loadAnns(ann_ids_3)

        return self._parse_ann_info(ann_info, self.with_mask), self._parse_ann_info(ann_info_2, self.with_mask), self._parse_ann_info(ann_info_3, self.with_mask)
    ...
    def _filter_imgs(self, min_size=32):
        ...
        valid_inds_3 = []
        ids_with_ann_3 = set(_['image_id'] for _ in self.coco_3.anns.values())
        for i, img_info in enumerate(self.img_infos_3):
            if self.img_ids_3[i] not in ids_with_ann_3:
                continue
            if min(img_info['width'], img_info['height']) >= min_size:
                valid_inds_3.append(i)
        return valid_inds, valid_inds_2, valid_inds_3
    ...
    def _parse_ann_info(self, ann_info, with_mask=True):
        ...
        for i, ann in enumerate(ann_info):
            ...
            if with_mask:
                ...
                # TODO: Inefficient, we only need to do this for original scale mask only!!!!
    ...
    def prepare_train_img(self, idx):
        ...
        img_info_3 = self.img_infos_3[idx]

        # ForkedPdb().set_trace()

        # ensure it's the same image but at different scales
        assert img_info['filename'] == img_info_2['filename'] == img_info_3['filename']

        img_file_path = osp.join(self.img_prefix, img_info['filename'])
        img_file_path_2 = osp.join(self.img_prefix_2, img_info_2['filename'])
        img_file_path_3 = osp.join(self.img_prefix_3, img_info_3['filename'])
        orig_img = np.load(img_file_path)
        orig_img_2 = np.load(img_file_path_2)
        orig_img_3 = np.load(img_file_path_3)

        upscale_factor_2 = orig_img_2.shape[0]/orig_img.shape[0]
        upscale_factor_3 = orig_img_3.shape[0]/orig_img.shape[0]

        total_num_slices = orig_img.shape[2]

        ann, ann_2, ann_3 = self.get_ann_infos(idx)
        
        gt_bboxes = ann['bboxes']
        gt_bboxes_2 = ann_2['bboxes']
        gt_bboxes_3 = ann_3['bboxes']

        gt_labels = ann['labels']
        gt_labels_2 = ann_2['labels']
        gt_labels_3 = ann_3['labels']

        if 'masks' not in ann and 'masks' not in ann_2 and 'masks' in ann_3:
            gt_masks = ann_3['masks']
            gt_masks_2 = None
            gt_masks_3 = None
        elif 'masks' in ann and 'masks' in ann_2 and 'masks' in ann_3:
            gt_masks = ann['masks']
            gt_masks_2 = ann_2['masks']
            gt_masks_3 = ann_3['masks']
        else:
            gt_masks = None
            gt_masks_2 = None
            gt_masks_3 = None
        ...
        if len(gt_bboxes) == 0 or len(gt_bboxes_2) == 0 or len(gt_bboxes_3) == 0:
            return None
        ...
        if self.extra_aug is not None:
            ...
            orig_img_2 = resize(orig_img, (orig_img.shape[0]*upscale_factor_2, orig_img.shape[1]*upscale_factor_2, orig_img.shape[2]*upscale_factor_2))
            if gt_masks is not None:
                gt_masks_2 = []
                for cur_mask in gt_masks:
                    gt_masks_2.append(resize(cur_mask, (cur_mask.shape[0]*upscale_factor_2, cur_mask.shape[1]*upscale_factor_2, cur_mask.shape[2]*upscale_factor_2)))
                gt_masks_2 = np.array(gt_masks_2)
            gt_bboxes_2 = gt_bboxes * upscale_factor_2
            gt_labels_2 = gt_labels
            img_scale_2 = (orig_img_2.shape[0], orig_img_2.shape[1]) # disable scaling...

            orig_img_3 = resize(orig_img, (orig_img.shape[0]*upscale_factor_3, orig_img.shape[1]*upscale_factor_3, orig_img.shape[2]*upscale_factor_3))
            if gt_masks is not None:
                gt_masks_3 = []
                for cur_mask in gt_masks:
                    gt_masks_3.append(resize(cur_mask, (cur_mask.shape[0]*upscale_factor_3, cur_mask.shape[1]*upscale_factor_3, cur_mask.shape[2]*upscale_factor_3)))
                gt_masks_3 = np.array(gt_masks_3)
            gt_bboxes_3 = gt_bboxes * upscale_factor_3
            gt_labels_3 = gt_labels
            img_scale_3 = (orig_img_3.shape[0], orig_img_3.shape[1]) # disable scaling...
        else:
            # randomly sample a scale
            img_scale = random_scale(self.img_scales, self.multiscale_mode)
            img_scale_2 = random_scale(self.img_scales_2, self.multiscale_mode)
            img_scale_3 = random_scale(self.img_scales_3, self.multiscale_mode)
   
        total_num_slices = orig_img.shape[2]
        total_num_slices_2 = orig_img_2.shape[2]
        total_num_slices_3 = orig_img_3.shape[2]
        data = None
        ...
        first_iter = True
        for cur_slice in range(total_num_slices_3):
            img = orig_img_3[:,:,cur_slice]
            # convert Greyscale to RGB
            img = np.repeat(img[:, :, np.newaxis], 3, axis=2)

            img, img_shape, pad_shape, _ = self.img_transform(
                img, img_scale_3, flip, keep_ratio=self.resize_keep_ratio)

            img = img.copy()
            if first_iter:
                ori_shape = (img.shape[1], img.shape[2], 3)
                img_shape_3 = (*img_shape, total_num_slices_3)
                pad_shape = (*pad_shape, total_num_slices_3)
                img_meta = dict(
                    ori_shape=ori_shape,
                    img_shape=img_shape_3,
                    pad_shape=pad_shape,
                    scale_factor=scale_factor,
                    flip=flip,
                    image_id=img_info_3['id'])
                data['img_meta_3'] = DC(img_meta, cpu_only=True)
                first_iter = False
            self.insert_to_dict(data, 'imgs_3', img)
        ...
        gt_bboxes_3 = self.bbox_transform(gt_bboxes_3, (*img_shape_3, total_num_slices_3), scale_factor, flip)
        data['gt_bboxes'] = DC(to_tensor(gt_bboxes))
        data['gt_bboxes_2'] = DC(to_tensor(gt_bboxes_2))
        data['gt_bboxes_3'] = DC(to_tensor(gt_bboxes_3))
        
        if self.with_label:
            data['gt_labels'] = DC(to_tensor(gt_labels))
            data['gt_labels_2'] = DC(to_tensor(gt_labels_2))
            data['gt_labels_3'] = DC(to_tensor(gt_labels_3))
        if gt_masks is not None:
        ...
        if gt_masks_3 is not None:
            gt_masks_3 = self.mask_transform(gt_masks_3, pad_shape, scale_factor, flip, is3D=True)
            gt_masks_3 = gt_masks_3.transpose(0,3,1,2)
            data['gt_masks_3'] = DC(to_tensor(gt_masks_3.astype(np.uint8)), cpu_only=True)
        ...
        imgs_3 = np.array(data['imgs_3'])
        imgs_3 = imgs_3.transpose(1, 0, 2, 3)
        data['imgs_3'] = DC(to_tensor(imgs_3), stack=True)
    ...
    def prepare_test_img(self, idx):
        ...
        # find corresponding img_info for 1.25x dataset
        index_2 = -1
        ...
        index_3 = -1
        for i, cur_info in enumerate(self.img_infos_3):
            if cur_info['filename'] == img_info['filename']:
                index_3 = i
    
        img_info_2 = self.img_infos_2[index_2]
        img_info_3 = self.img_infos_3[index_3]
        patient_imgs = np.load((osp.join(self.img_prefix, img_info['filename'])))
        patient_imgs_2 = np.load((osp.join(self.img_prefix_2, img_info_2['filename'])))
        patient_imgs_3 = np.load((osp.join(self.img_prefix_3, img_info_3['filename'])))

        # scale_factor = 1.0 / (patient_imgs_3.shape[0] / patient_imgs.shape[0]) # scale up to img_info_3's resolution
        # scale_factor_2 = 1.0 / (patient_imgs_3.shape[0] / patient_imgs_2.shape[0]) # scale up to img_info_3's resolution
        # scale_factor_3 = 1 # remain the same

        scale_factor = 1
        scale_factor_2 = (patient_imgs_2.shape[0] / patient_imgs.shape[0])
        scale_factor_3 = (patient_imgs_3.shape[0] / patient_imgs.shape[0])

        total_num_slices = patient_imgs.shape[2]
        total_num_slices_2 = patient_imgs_2.shape[2]
        total_num_slices_3 = patient_imgs_3.shape[2]
        ...
        img_metas_3 = []
        for cur_slice in range(total_num_slices_3):
            img_3 = patient_imgs_3[:,:,cur_slice]
            # convert Greyscale to RGB
            img_3 = np.repeat(img_3[:, :, np.newaxis], 3, axis=2)

            for scale in self.img_scales_3:
                _img, _img_meta, _proposal = prepare_single(
                    img_3, scale, False, img_info_3, total_num_slices_3, scale_factor_3)
                imgs_3.append(_img)
                if len(img_metas_3) == 0:
                    img_metas_3.append(DC(_img_meta, cpu_only=True))
        ...
        imgs_3 = np.transpose(imgs_3, (1, 0, 2, 3))

        assert imgs.shape[0] == 3 and imgs_2.shape[0] == 3 and imgs_3.shape[0] == 3     # make sure [0] is the number of channels
        data = dict(imgs=to_tensor(imgs), img_meta=img_metas, imgs_2=to_tensor(imgs_2), img_meta_2=img_metas_2, imgs_3=to_tensor(imgs_3), img_meta_3=img_metas_3)
