...
class TwoStageDetector3D23calesOnePathway(BaseDetector, RPNTestMixin, BBoxTestMixin,
                       MaskTestMixin):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(TwoStageDetector3D23calesOnePathway, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(TwoStageDetector3D23calesOnePathway, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            bbox_targets_3 = self.bbox_head.get_target(
                sampling_results_3, gt_bboxes_3, gt_labels_3, self.train_cfg.rcnn)
            bbox_targets_combined = []
            for bbox_target, bbox_target_2, bbox_target_3 in zip(bbox_targets, bbox_targets_2, bbox_targets_3):
                bbox_targets_combined.append(torch.cat((bbox_target, bbox_target_2, bbox_target_3), 0))
            ...
            upscaled_factor = img_meta_3[0]['ori_shape'][0] / img_meta[0]['ori_shape'][0]
            pred_bboxes_3 = self.bbox_head.convert_adjustments_to_bboxes(rois_3, bbox_pred_3, img_meta_3[0]['img_shape'])[:,6:].cpu().detach().numpy() / upscaled_factor
            ...
            bbox_feats_refinement_2 = self.bbox_roi_extractor_refinement(
                x[:self.bbox_roi_extractor_refinement.num_inputs], rois_refinement_2)
            ...
            refined_bbox_pred_combined = torch.cat((refined_bbox_pred, refined_bbox_pred_2))
            ...
            for bbox_target_refinement, bbox_target_refinement_2 in zip(bbox_targets_refinement, bbox_targets_refinement_2):
                bbox_targets_refinement_combined.append(torch.cat((bbox_target_refinement, bbox_target_refinement_2)))

            loss_refinement = self.refinement_head.loss(refined_bbox_pred_combined,
                                            *bbox_targets_refinement_combined)
        ...
        if self.with_mask:
            ...
            mask_targets_combined = torch.cat((mask_targets, mask_targets_refined))
            pos_labels_combined = torch.cat((pos_labels, pos_labels_refined))
            loss_mask = self.mask_head.loss(mask_pred_combined, mask_targets_combined,
                                            pos_labels_combined)
    ...
    def simple_test(self, imgs, img_metas, imgs_2, img_metas_2, imgs_3, img_metas_3, pp=None, pp_2=None, proposals=None, rescale=False, test_cfg2=None, test_cfg3=None):
        ...
        bboxes_3, scores_3 = self.simple_test_bboxes(
            x_3, img_metas_3, proposal_list_3, None, rescale=rescale)
        ...
        bboxes = torch.cat((bboxes, bboxes_2_refinement, bboxes_3_refinement), 0)
